...
def test_image(color, img_index, img_type):
    ...
    while(1):
        ...
        res, mask, x_center_difference = detect_notch_line(hsv, frame, mask, res, box_center, cv2.contourArea(box))
...
def detect_notch_line(hsv, frame, og_mask, og_res, box_center, box_area):
    mask = cv2.inRange(hsv, np.array([0, 0, 0]), np.array([180, 255, 15]))
    res = cv2.bitwise_and(frame,frame, mask= mask)
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    filtered_contours = [cnt for cnt in contours if (cv2.contourArea(cnt) > 100 and 4 < (box_area / cv2.contourArea(cnt)) < 50)]
    line_notch_center = -1
    try:
        filtered_contours.sort(key=lambda cnt: cv2.contourArea(cnt), reverse=True)
...
if __name__ == "__main__":
    test_image(RED, img_index=6, img_type="jpg")
